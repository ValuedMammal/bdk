...
fn sync_returns_chain_and_graph_update() -> Result<()> {
    ...
    let mut graph = IndexedTxGraph::<ConfirmationBlockTime, KeychainTxOutIndex<Keychain>>::new({
    ...
    // chain updated
    assert_eq!(chain_changeset.blocks.len(), 10);
    let cp = chain.tip();
    assert_eq!(cp.height(), remote_tip_height);

    // new tx added
    let graph_init_changeset = indexed_tx_graph.initial_changeset();
    let tx = graph_init_changeset.tx_graph.txs.iter().next().unwrap();
    let txout = tx
        .output
        .iter()
    ...
    // new anchor added
    let (anchor, txid) = graph_init_changeset.tx_graph.anchors.iter().next().unwrap();
    ...
    assert_eq!(balance.confirmed, send_amt);
