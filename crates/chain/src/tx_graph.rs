...
//!
//! 1. **Create a [`CanonicalTask`]** using [`canonical_task`](TxGraph::canonical_task): ```ignore
//!    let task = tx_graph.canonical_task(tip, params); ``` This encapsulates all of the
//!    canonicalization logic without performing any I/O.
//!
//! 2. **Execute the task** with a chain oracle to obtain a [`CanonicalView`]: ```ignore let view =
//!    chain.canonicalize(task).view(); ``` The chain oracle (such as
//!    [`LocalChain`](crate::local_chain::LocalChain)) answers the anchor verification queries that
//!    the task emits.
//!
...
impl<A> From<TxNode<'_, Arc<Transaction>, A>> for Arc<Transaction> {
    fn from(tx_node: TxNode<'_, Arc<Transaction>, A>) -> Self {
        tx_node.tx.clone()
    }
}
