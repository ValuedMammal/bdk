...
impl Wallet {
    ...
    pub fn build_fee_bump(
        ...
        for txin in original_txin {
            if let Some(out) = local_outputs.get(&txin.previous_output) {
...
impl Wallet {
    ...
    pub(crate) fn build_psbt(
        ...
        let current_height = match assets.absolute_timelock {
        ...
        let mut unspent = vec![];
        if !params.manually_selected_only {
            unspent.extend(self.filter_utxos(self.list_unspent(), &params, current_height));
            // sorting available utxos
            use tx_builder::CoinSelectionStrategy::*;
            match params.coin_selection {
                OldestFirst => {
                    unspent.sort_by_key(|output| output.chain_position);
                }
                LargestFirst => {
                    unspent.sort_by_key(|output| output.txout.value);
                    unspent.reverse();
                }
                BranchAndBound | SingleRandomDraw => {
                    utils::shuffle_slice(&mut unspent, rng);
                }
            }
        }
            ...
            || (plan_utxos.is_empty() && !unspent.is_empty())
        {
            ...
            for output in params.utxos.iter().chain(&unspent) {
                let _ = self
        ...
        let candidates: Vec<WeightedUtxo> = manually_selected
            ...
            .chain(params.foreign_utxos.clone())
            .chain(unspent.iter().flat_map(|out| {
            ...
            match wutxo.utxo {
                ...
                Utxo::Foreign {
                    ...
                    builder
                        ...
                        .map_err(CreateTxError::BuildPsbt)?;
                }
            }
        }
        ...
        updater
            ...
            .map_err(CreateTxError::UpdatePsbt)?;
