...
Another idea that was offered was to return something implementing `Future` from methods like `commit`. The idea was that it would minimize added dependencies and increase flexiblity by allowing the caller `await` the result. In the end it seems less of an effort was put toward executing this idea.
...
It remains the job of the developers to adequately document and showcase a correct use of the API. It is important that we clearly define the structure of the changeset we now expect users to be aware of and to be proactive and transparent when communicating upgrades to the data model.
